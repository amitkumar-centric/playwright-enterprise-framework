...
export class PimPage {
  ...
  constructor(private readonly page: Page) {
    this.addEmployeeMenu = page.getByText('Add Employee', {
      exact: true
    });
  ...
  private async waitForLoaderToSettle(): Promise<void> {
    await this.formLoader.waitFor({
      state: 'hidden'
    });
  }
  ...
  async gotoOrangeHrmEmployeeList(): Promise<void> {
    await this.page.goto(
      PimPage.buildOrangeHrmUrl('/web/index.php/pim/viewEmployeeList')
    );

    await this.addEmployeeMenu.waitFor();
  ...
  async createEmployee(
    ...
    try {
      ...
      await this.employeeIdInput.fill(employeeId ?? faker.string.numeric(6));
